...
namespace Crayon
{
	...
	void Application::OnEvent(Event* event)
	{
		...
		// Mouse Button
		if (MouseButtonEvent* mbEvent = dynamic_cast<MouseButtonEvent*>(event))
		{
		...
		// Mouse Movement
		if (MouseMovedEvent* mmEvent = dynamic_cast<MouseMovedEvent*>(event))
		{
			this->OnMouseMoved(mmEvent->GetX(), mmEvent->GetY());
		}

		// Window Events
		if (WindowEvent* wdEvent = dynamic_cast<WindowEvent*>(event))
		{
